...
def _executeTransaction_0(infos = False):
    ...
    solver__executeTransaction_0.add(And(Or('' in O_role,'' in Owner_role), ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,minAppr,_transaction], Implies(And(len(approvedOwners) >= minAppr,And(executedTransaction.eq(_transaction))), Or(Exists([_owner], True))))))
    ...
    if infos :
        print("--For _executeTransaction_0: ", simplify(And(Or('' in O_role,'' in Owner_role), ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,minAppr,_transaction], Implies(And(len(approvedOwners) >= minAppr,And(executedTransaction.eq(_transaction))), Or(Exists([_owner], True)))))), " :: ", result)

        if  not eps_result :
            print ("Non deterministic: ", simplify(True))
            
        if not result: 
            solver__executeTransaction_02.add(Not(And(Or('' in O_role,'' in Owner_role), ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,minAppr,_transaction], Implies(And(len(approvedOwners) >= minAppr,And(executedTransaction.eq(_transaction))), Or(Exists([_owner], True)))))))
            print("\nSimplify of the Not Formula: ", simplify(Not(And(Or('' in O_role,'' in Owner_role), ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,minAppr,_transaction], Implies(And(len(approvedOwners) >= minAppr,And(executedTransaction.eq(_transaction))), Or(Exists([_owner], True))))))), " :: ", solver__executeTransaction_02.check() == z3.sat)
...
def _executeTransaction_1(infos = False):
    ...
    solver__executeTransaction_1.add(And(Or('' in O_role), ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,minAppr], Implies(And(len(approvedOwners) >= minAppr,And(executedTransaction.eq(pendingTransactions[-1]))), Or(Exists([_owner], True))))))
    ...
    if infos :
        print("--For _executeTransaction_1: ", simplify(And(Or('' in O_role), ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,minAppr], Implies(And(len(approvedOwners) >= minAppr,And(executedTransaction.eq(pendingTransactions[-1]))), Or(Exists([_owner], True)))))), " :: ", result)

        if  not eps_result :
            print ("Non deterministic: ", simplify(True))
            
        if not result: 
            solver__executeTransaction_12.add(Not(And(Or('' in O_role), ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,minAppr], Implies(And(len(approvedOwners) >= minAppr,And(executedTransaction.eq(pendingTransactions[-1]))), Or(Exists([_owner], True)))))))
            print("\nSimplify of the Not Formula: ", simplify(Not(And(Or('' in O_role), ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,minAppr], Implies(And(len(approvedOwners) >= minAppr,And(executedTransaction.eq(pendingTransactions[-1]))), Or(Exists([_owner], True))))))), " :: ", solver__executeTransaction_12.check() == z3.sat)
...
check_resut = (_initializeWallet_0() and _submitTransaction_0() and _approveTransaction_0() and _executeTransaction_0() and _executeTransaction_1() and _resetWallet_0())
