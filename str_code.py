...
def _start_0(reset = False):
    ...
    if match.group(0) in globals():
        ...
        state = True
        solver__start_0.add(state == True)
    else:
...
def _makeoffer_0(reset = False):
    ...
    if match.group(0) in globals():
        # If the variable exists, create a valid assignment
        price  =  _p
        solver__makeoffer_0.add(price  ==  _p)
    else:
        raise NameError(f"State Variable '{match.group(0)}' does not exist")
...
def _acceptoffer_0(reset = False):
    ...
    #check if precondition condition and the or of all direived preconditions id true 
    solver__acceptoffer_0.add(And(_pre == z3.sat, Or(And(Not(state), And(forall_in_set(formula_0, M), exist_in_set(formula_1, M))),True)))
...
def _rejectoffer_0(reset = False):
    ...
    #and()sjjd do not meet the assignations requirements
    solver__rejectoffer_0.add(False)              
...
def _OK_0(reset = False):
    ...
    #set the stack init
    solver__OK_0.push()
    solver__OK_0.add(And(Not(state), And(forall_in_set(formula_2, M), exist_in_set(formula_3, M))))
    #getting the check result of the precondition
