...
def _executeTransaction_0(minimize = False):
    ...
    #check if post condition implies any pre precondition
    solver__executeTransaction_0.add(ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,_transaction], Implies(And(executedTransaction  ==  _transaction), Or(Exists([_owner], True)))))
    result = solver__executeTransaction_0.check() == z3.sat
    if minimize :
        print("--For _executeTransaction_0: ", simplify(ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,_transaction], Implies(And(executedTransaction  ==  _transaction), Or(Exists([_owner], True))))), " :: ", result)
        if not result: 
            solver__executeTransaction_02.add(Not(ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,_transaction], Implies(And(executedTransaction  ==  _transaction), Or(Exists([_owner], True))))))
            print("\nMinify of the Not Formula: ", simplify(Not(ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty,_transaction], Implies(And(executedTransaction  ==  _transaction), Or(Exists([_owner], True)))))), " :: ", solver__executeTransaction_02.check() == z3.sat)
...
def _executeTransaction_1(minimize = False):
    ...
    #check if post condition implies any pre precondition
    solver__executeTransaction_1.add(ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty], Implies(And(executedTransaction  ==  pendingTransactions[-1]), Or(Exists([_owner], True)))))
    result = solver__executeTransaction_1.check() == z3.sat
    if minimize :
        print("--For _executeTransaction_1: ", simplify(ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty], Implies(And(executedTransaction  ==  pendingTransactions[-1]), Or(Exists([_owner], True))))), " :: ", result)
        if not result: 
            solver__executeTransaction_12.add(Not(ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty], Implies(And(executedTransaction  ==  pendingTransactions[-1]), Or(Exists([_owner], True))))))
            print("\nMinify of the Not Formula: ", simplify(Not(ForAll([owners,pendingTransactions,approvedOwners,executedTransaction,empty], Implies(And(executedTransaction  ==  pendingTransactions[-1]), Or(Exists([_owner], True)))))), " :: ", solver__executeTransaction_12.check() == z3.sat)
