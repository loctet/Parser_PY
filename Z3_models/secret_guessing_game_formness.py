...
def _makeGuess_0(infos = False):
    ...
    #solver__makeGuess_0.add(And(Not(gameWon), Not(_guess.eq(secret))))
    solver__makeGuess_0.add(And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess], Implies(And(And(Not(gameWon), Not(_guess.eq(secret))),True), Or(Exists([_guess], And(Not(gameWon), Not(_guess.eq(secret)))),Exists([_guess], And(Not(gameWon), _guess.eq(secret))))))))
    post_result = solver__makeGuess_0.check() == z3.sat
    #print((And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess], Implies(And(And(Not(gameWon), Not(_guess.eq(secret))),True), Or(Exists([_guess], And(Not(gameWon), Not(_guess.eq(secret)))),Exists([_guess], And(Not(gameWon), _guess.eq(secret)))))))), post_result)
    ...
    if infos :
        print("--For _makeGuess_0: ", simplify(And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess], Implies(And(And(Not(gameWon), Not(_guess.eq(secret))),True), Or(Exists([_guess], And(Not(gameWon), Not(_guess.eq(secret)))),Exists([_guess], And(Not(gameWon), _guess.eq(secret)))))))), " :: ", result)

        if  not eps_result :
            print ("Non deterministic: ", simplify(And(ForAll([_guess], Implies(And(Not(gameWon), Not(_guess.eq(secret))), And(Not(And(Not(gameWon), _guess.eq(secret)))))),ForAll([_guess], Implies(And(Not(gameWon), _guess.eq(secret)), And(Not(And(Not(gameWon), Not(_guess.eq(secret))))))))))
            
        if not result: 
            solver__makeGuess_02.add(Not(And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess], Implies(And(And(Not(gameWon), Not(_guess.eq(secret))),True), Or(Exists([_guess], And(Not(gameWon), Not(_guess.eq(secret)))),Exists([_guess], And(Not(gameWon), _guess.eq(secret)))))))))
            print("\nSimplify of the Not Formula: ", simplify(Not(And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess], Implies(And(And(Not(gameWon), Not(_guess.eq(secret))),True), Or(Exists([_guess], And(Not(gameWon), Not(_guess.eq(secret)))),Exists([_guess], And(Not(gameWon), _guess.eq(secret))))))))), " :: ", solver__makeGuess_02.check() == z3.sat)
...
def _makeGuess_1(infos = False):
    ...
    #solver__makeGuess_1.add(And(Not(gameWon_old), _guess.eq(secret)))
    solver__makeGuess_1.add(And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess,gameWon_old], Implies(And(And(Not(gameWon_old), _guess.eq(secret)),And(gameWon == True)), True))))
    post_result = solver__makeGuess_1.check() == z3.sat
    #print((And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess,gameWon_old], Implies(And(And(Not(gameWon_old), _guess.eq(secret)),And(gameWon == True)), True)))), post_result)
    ...
    if infos :
        print("--For _makeGuess_1: ", simplify(And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess,gameWon_old], Implies(And(And(Not(gameWon_old), _guess.eq(secret)),And(gameWon == True)), True)))), " :: ", result)

        if  not eps_result :
            print ("Non deterministic: ", simplify(True))
            
        if not result: 
            solver__makeGuess_12.add(Not(And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess,gameWon_old], Implies(And(And(Not(gameWon_old), _guess.eq(secret)),And(gameWon == True)), True)))))
            print("\nSimplify of the Not Formula: ", simplify(Not(And('Guesser' in ['Guesser'], ForAll([secret,gameWon,_guess,gameWon_old], Implies(And(And(Not(gameWon_old), _guess.eq(secret)),And(gameWon == True)), True))))), " :: ", solver__makeGuess_12.check() == z3.sat)
